...
public class EstoqueBean {
	...
	private Logs validarCampos() {
		...
		try {
			if (item.getCodigoProduto()==null && ("".equals(item.getDescricaoProduto()) || item.getDescricaoProduto()==null) 
				...
				log.logAcao(null, FacesMessage.SEVERITY_INFO, "Campos Obrigatorios!","Campos Obrigatorios não preenchidos!");

			} 	  else if ("".equals(item.getCodigoProduto().toString()) ||
				  item.getCodigoProduto() == null) 
			      { log.logAcao("frm_estoque:ipt_codigo",
				  FacesMessage.SEVERITY_INFO, summaryDefault, "Codigo do Produto deve ser informado!");				  
				  }
	...
	public List<ItemEstoque> getListaProdutos() {
		return listaProdutos;
	}

	public void setListaProdutos(List<ItemEstoque> listaProdutos) {
